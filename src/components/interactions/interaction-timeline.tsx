...
export function InteractionTimeline({ leadId }: InteractionTimelineProps) {
  ...
  if (isError) {
    return <div className="py-8 text-center text-sm text-danger">Failed to load timeline</div>;
  }
  ...
  if (filtered.length === 0) {
    return <div className="py-8 text-center text-sm text-muted">No interactions recorded yet</div>;
  }
        ...
        {filtered.map((interaction) => (
          ...
          >
            ...
            <div className="mb-2 flex items-center gap-2">
              ...
              {interaction.outcome && (
                <span className="text-xs capitalize text-muted">{interaction.outcome}</span>
              )}
            ...
            {interaction.notes && (
              <p className="text-sm leading-relaxed text-muted">{interaction.notes}</p>
            )}
          </div>
        ))}
      </div>
