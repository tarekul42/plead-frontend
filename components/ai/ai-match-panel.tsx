  ...
  return (
    <div className="rounded-card border border-border bg-surface p-6 shadow-sm">
      ...
      {!enabled ? (
        <button
          onClick={handleMatch}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#10B981] px-4 py-2.5 text-sm font-medium text-white transition hover:opacity-90"
        >
        ...
        <div className="rounded-lg bg-danger/5 p-4 text-sm">
          ...
          <button onClick={handleMatch} className="mt-2 text-xs text-[#2563EB] hover:underline">
            Retry
          </button>
        </div>
      ) : (
        <div className="space-y-3">
            ...
            <div key={i} className="rounded-lg border border-border bg-background p-3">
              <div className="mb-1 flex items-center justify-between">
                ...
                <span className={`text-xs font-bold ${
                  match.score >= 80 ? "text-[#10B981]" : match.score >= 60 ? "text-[#F59E0B]" : "text-muted"
                }`}>
                  {match.score}%
                </span>
              </div>
                ...
                <ul className="space-y-0.5">
                  {match.reasons.map((r: string, j: number) => (
                    <li key={j} className="flex items-start gap-1.5 text-xs text-muted">
                      <span className="mt-0.5 h-1 w-1 shrink-0 rounded-full bg-[#10B981]" />
                      {r}
                    </li>
                  ))}
                </ul>
          ...
          <button onClick={handleMatch} className="w-full text-xs text-[#2563EB] hover:underline">
            Refresh matches
          </button>
        </div>
      )}
